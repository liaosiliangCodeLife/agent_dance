...
15|| 网络 | `http` 包（普通请求）+ 手动 SSE 解析（`http.Client.send()` 流式读 body）；**API 端点使用 `/v1/chat/completions`，客户端管理 `messages` 历史数组** |
...
### 背景
之前 F-126 约束单轮对话，API 请求只传当前消息，AI 不知道之前聊过什么。v3.0 升级为多轮。

### 实现方案（推荐方案 A）

#### 方案 A：切到 /v1/responses 端点 ✅
- 第一轮：POST /v1/responses  { "input":"用户输入", "instructions":"系统提示词", "store":true }
- 返回 { "id":"resp_abc", "output":[...] }
- 第二轮：POST /v1/responses  { "input":"继续聊", "previous_response_id":"resp_abc" }
- 客户端只需存一个 previousResponseId 字符串，不用拼接 messages

#### 方案 B：继续用 /v1/chat/completions + 客户端拼历史
- 客户端维护 List<Map> 类型的 chatHistory
- 每轮把 assistant 回复 push 进去，请求时完整发送

### 需改动的文件
| 文件 | 改动 |
|------|------|
| agents_api_client.dart | 支持 previous_response_id（方案A）或 messages 拼接（方案B） |
| chat_repository.dart | 发送携带上下文，接收解析 output（方案A） |
| chat_viewmodel.dart | 管理对话历史状态 |

### 验收标准
- 连续发言"我叫张三"→"我叫什么"→"加上年龄25"，AI 正确回答前面内容
- 切换服务器再回来，对话历史还在
