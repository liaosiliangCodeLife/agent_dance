...
三步上手 Agent Dance，零门槛从零到对话。

### 🖥️ 第一步：下载并启动桌面端服务

Agent Dance 需要一台电脑作为智能体服务端（你的 AI 网关），请在 Windows 或 macOS 上下载对应的桌面端应用：

| 平台 | 下载方式 |
|------|----------|
| **Windows** | 前往 [Releases 页面](https://github.com/liaosiliangCodeLife/agent_dance/releases) → 下载 `agent_dance_server_windows_x64.exe` |
| **macOS** | 前往 [Releases 页面](https://github.com/liaosiliangCodeLife/agent_dance/releases) → 下载 `agent_dance_server_macos.dmg` |

> 如果你已在电脑上部署 [Hermes Agent](https://hermes-agent.nousresearch.com)，可直接启用其 API Server，作为桌面端服务，无需额外下载。

### ⚙️ 第二步：配置并启动

打开电脑端 App，你会看到配置界面，只需填写以下三项：

| 配置项 | 说明 |
|--------|------|
| **Server** | 你的个人服务器地址。局域网内填 `0.0.0.0` 或本机局域网 IP（如 `192.168.1.100`）；如需公网穿透，填写 NPS / frp 隧道地址 |
| **Key** | 自定义 API 密钥，由你自己设定。手机端连接时需要提供相同的 Key 作为鉴权凭证 |
| **协议** | 默认 **TCP** 传输，稳定可靠 |

配置完成后，点击 **启动服务**，桌面端即开始监听来自手机端的连接请求。

> 配置界面参考：Server 填写个人服务器地址 → Key 填写自定义密钥 → 协议保持默认 TCP → 启动服务

### 📱 第三步：手机端连接

在手机上安装 Agent Dance App，连接到刚才配置好的桌面服务：

1. **安装 App**：从 [Releases 页面](https://github.com/liaosiliangCodeLife/agent_dance/releases) 下载 APK 安装到 Android 手机（iOS 用户可通过 TestFlight 安装）
2. **添加服务器**：打开 App → 底部 **服务器** Tab → 点击右下角 **+**
3. **填写连接信息**：
   - **名称**：给你的服务器起个名字（如"家里的电脑"、"办公室主机"）
   - **地址**：填写桌面端设定的 Server 地址（局域网填 `192.168.x.x`，远程填隧道地址）
   - **端口**：默认 `8642`
   - **API Key**：填写桌面端设定的 Key
4. **测试连接**：点击 **测试连接** 按钮，验证通过后自动保存配置
5. **开始对话**：回到 **智能体** Tab → 点击刚添加的服务器 → 输入你的第一条消息

> 💡 同一局域网下延迟最低，体验最佳。远程连接需配合 NPS / frp 等隧道工具穿透内网，或使用公网 IP。
